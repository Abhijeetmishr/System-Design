package LLD.DesignSmartHomeSystem;

import java.util.*;

import LLD.DesignSmartHomeSystem.SmartHome.Light;
import LLD.DesignSmartHomeSystem.SmartHome.SmartHomeDevice;
import LLD.DesignSmartHomeSystem.SmartHome.SmartHomeDeviceFactory;
import LLD.DesignSmartHomeSystem.SmartHome.SmartHomeDeviceFactoryImpl;
import LLD.DesignSmartHomeSystem.Interface.InterfaceDevice;
import LLD.DesignSmartHomeSystem.Interface.InterfaceDeviceFactory;
import LLD.DesignSmartHomeSystem.Interface.InterfaceDeviceFactoryImpl;

public class HomeAutomationSystem {
    private Map<String, InterfaceDevice> interfaceDevices = new HashMap<>();
    private Map<String, SmartHomeDevice> smartHomeDevices = new HashMap<>();
    private Map<String, Set<String>> deviceInterfaceMapping = new HashMap<>();

    private InterfaceDeviceFactory interfaceDeviceFactory = new InterfaceDeviceFactoryImpl();
    private SmartHomeDeviceFactory smartHomeDeviceFactory = new SmartHomeDeviceFactoryImpl();
    ...
    public void addInterfaceDevice(String name, String location, String activationKeyword) {
        ...
        if (!interfaceDevices.containsKey(uniqueKey)) {
            InterfaceDevice interfaceDevice = interfaceDeviceFactory.createInterfaceDevice(name, activationKeyword);
            if (interfaceDevice != null) {
                interfaceDevices.put(uniqueKey, interfaceDevice);
                System.out.println("Added Interface Device: " + name + " at " + location);
            }
        } else {
            System.out.println("Invalid Input: Interface device with this input and name already exists.");
        }
    }
    ...
    public void giveCommand(String activationKeyword, String deviceName, String location, String commandValue) {
        ...
        if (isDeviceConnectedToInterface(deviceIdentifier, activationKeyword)) {
            ...
            System.out.println("The device " + deviceName + " at " + location
                    + " is not connected to an interface with the activation keyword '" + activationKeyword + "'.");
        }
    }
    ...
    public void printConnectedDevices(String activationKeyword, String location) {
        ...
        smartHomeDevices.forEach((deviceIdentifier, device) -> {
            if (deviceIdentifier.endsWith("@" + location)
                    && isDeviceConnectedToInterface(deviceIdentifier, activationKeyword)) {
    ...
    private boolean isDeviceConnectedToInterface(String deviceIdentifier, String activationKeyword) {
        ...
        if (deviceInterfaceMapping.containsKey(deviceIdentifier)) {
            return deviceInterfaceMapping.get(deviceIdentifier).stream()
                    .anyMatch(interfaceId -> interfaceDevices.get(interfaceId).getActivationKeyword()
                            .equals(activationKeyword));
        }
        return false;
    }
    ...
    public void connectSmartHomeDevice(String activationkeyword, String SmartHomedeviceName, String location) {
        ...
        if (smartHomeDevices.containsKey(deviceIdentifier) && interfaceDevices.containsKey(interfaceIdentifier)) {
            ...
            System.out.println("Connected " + SmartHomedeviceName + " at " + location + " to "
                    + getInterfaceDeviceName(activationkeyword, location));
    ...
    public String getInterfaceDeviceName(String activationKeyword, String location) {
        ...
        if (interfaceDevices.containsKey(interfaceIdentifier)) {
            ...
            System.out.println("Interface Device with activation keyword '" + activationKeyword
                    + "' not found at location " + location);
            return null;
        }
    }
